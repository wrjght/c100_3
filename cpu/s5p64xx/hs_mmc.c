...
static void set_clock (uint clksrc, uint div)
{
	...
	tmp_control2 = 0xC0004100 | (clksrc << 4);
	tmp_control2 &= ~(1<<15);
	tmp_control2 |= (1<<14);
	tmp_control3 = 0x0;
	tmp_control4 = (0x3 << 16);

	writel(tmp_control2, regs + HM_CONTROL2);	// rx feedback control
	writel(tmp_control3, regs + HM_CONTROL3); 			// Low clock: 00008080
	writel(tmp_control4, regs + HM_CONTROL4);
