 ...
 *
 *  $Id: usbd-otg-hs.c,v 1.1 2009/03/10 07:09:55 charles Exp $
 *
 * (C) Copyright 2007
...
void s3c_usb_reset(void)
{
	...
	/* IN ep TxFIFO in dedicated FIFO */
	for (i=0; i<(S3C_MAX_ENDPOINTS-1); i++)
		writel(NPTX_FIFO_SIZE<<16| (NPTX_FIFO_START_ADDR + NPTX_FIFO_SIZE + PTX_FIFO_SIZE*i)<<0, S3C_OTG_DPTXFSIZ1+4*i);
